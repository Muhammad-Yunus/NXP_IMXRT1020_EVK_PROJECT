...
#define RED_565     0xF800   // RGB565 for red
...
void ILI9341_SetWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ...
    LCD_WriteMultiData(data_col, sizeof(data_col));

    LCD_WriteCmd(0x2B); // Row address set
    uint8_t data_row[] = { y0 >> 8, y0 & 0xFF, y1 >> 8, y1 & 0xFF };
    LCD_WriteMultiData(data_row, sizeof(data_row));
...
static void LCD_FlushDisplay(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t  *color_p)
{
    ...
    LCD_WriteMultiData((const uint8_t *)color_p, send_size);
...
static void LCD_ReadTouch(lv_indev_drv_t  *drv, lv_indev_data_t *data)
{
...
void lv_port_indev_init(void)
{
    ...
    indev_drv.read_cb = LCD_ReadTouch;
